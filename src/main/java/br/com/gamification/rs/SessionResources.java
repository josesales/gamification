...
import br.com.gamification.service.SessionService;
...
public class SessionResources {
	...
	public Response save(JsonSession jsonSession) {
		try {
			...
			return Response.serverError().entity(new JsonError(e, message, jsonSession, e.getLegalMessage())).build();
		} catch (Exception e) {
			String message = String.format("Não foi possivel salvar  session [ %s ] parametros [ %s ]", e.getMessage(), jsonSession.toString());
			LOGGER.error(message, e);
			return Response.serverError().entity(new JsonError(e, message, jsonSession)).build();
		}
	}
