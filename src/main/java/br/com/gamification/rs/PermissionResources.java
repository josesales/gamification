...
import br.com.gamification.service.PermissionService;
...
public class PermissionResources {
	...
	public Response save(JsonPermission jsonPermission) {
		try {
			...
			return Response.serverError().entity(new JsonError(e, message, jsonPermission, e.getLegalMessage())).build();
		} catch (Exception e) {
			String message = String.format("Não foi possivel salvar  permission [ %s ] parametros [ %s ]", e.getMessage(), jsonPermission.toString());
			LOGGER.error(message, e);
			return Response.serverError().entity(new JsonError(e, message, jsonPermission)).build();
		}
	}
