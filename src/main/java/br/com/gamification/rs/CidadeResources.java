...
import br.com.gamification.service.CidadeService;
...
public class CidadeResources {
	...
	public Response save(JsonCidade jsonCidade) {
		try {
			...
			return Response.serverError().entity(new JsonError(e, message, jsonCidade, e.getLegalMessage())).build();
		} catch (Exception e) {
			String message = String.format("Não foi possivel salvar  cidade [ %s ] parametros [ %s ]", e.getMessage(), jsonCidade.toString());
			LOGGER.error(message, e);
			return Response.serverError().entity(new JsonError(e, message, jsonCidade)).build();
		}
	}
